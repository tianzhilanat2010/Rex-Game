...
{
	public AnimationClip animation; //If an animation is slotted, it will play it and despawn once it finishes
	...
	protected IEnumerator PlayCoroutine()
	{
		...
		else //If no animation is slotted, check for an attached ParticleSystem and play that before despawning
		{
			...
			{
				particleSystem.Play();
				durationBeforeDespawn = particleSystem.startLifetime;
			}
		}
